...
Downloads FileList.json from a repository and then downloads all files in the structure.
Supports both direct FileList URLs and dataset DOI URLs.
"""
...
class DatasetDownloader:
    """Download complete datasets from repository URLs."""
    ...
    def download_filelist(self, url: str, output_path: Optional[str] = None) -> Dict:
        """
        Download FileList.json from a URL.
        
        Args:
            url: URL to download FileList.json from
    ...
    def _download_tree_recursive(self, base_path: Path, tree: Dict,
        ...
        for file_info in tree.get('files', []):
            ...
            if 'url' not in file_info:
                self._log(f"[{current_file}/{total_files}] ⚠ Skipping {filename} - no URL available", "warning")
                self.stats['files_skipped'] += 1
                continue
            
            # Skip if file exists and skip_existing is True
            if skip_existing and file_path.exists():
                self._log(f"[{current_file}/{total_files}] ⊙ Skipping {filename} - already exists")
                self.stats['files_skipped'] += 1
                continue
            ...
            if delay > 0 and current_file < total_files:
                time.sleep(delay)
        ...
        if create_structure:
            for dir_name, subtree in tree.get('directories', {}).items():
                ...
                current_file = self._download_tree_recursive(
...
def download_complete_dataset(filelist_url: Optional[str] = None,
    ...
    Args:
        filelist_url: URL to download FileList.json from
    ...
    if filelist_url:
        ...
        downloader.download_filelist(filelist_url, filelist_save_path)
    elif filelist_path:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Download complete dataset from repository",
...
Examples:
  ...
  # Download without folder structure (all files in one directory)
  python dataset_downloader.py -u "https://example.com/filelist" -o ./data --no-structure
  
  # Force re-download existing files
  python dataset_downloader.py -f FileList.json -o ./data --no-skip
        """
    )
    ...
    input_group.add_argument(
        "-u", "--url",
        help="URL to download FileList.json from"
    )
    input_group.add_argument(
        "-f", "--file",
